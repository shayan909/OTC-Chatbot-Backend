...
# Convert to a specific JSON structure for quickReplies
...
def getOptions(inputSymptoms):
    extractedCol = set()
    diseases = pd.read_csv("Disease Dataset Multilabel.csv")
    df = pd.DataFrame(diseases)
    # Convert Dataset column names and input to lower case
    df.columns = df.columns.str.lower()
    inputSymptoms = [each_string.lower() for each_string in inputSymptoms]
    ...
    # Removing elements that are found in input and extracted Column names
    for element in inputSymptoms:
        if element in extractedCol:
            extractedCol.remove(element)
